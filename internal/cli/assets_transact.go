...
func (m transactModel) renderSummary() string {
	...
	if m.txType == sellTransaction {
		...
		if !exists {
			m.err = fmt.Errorf("asset %s not found in wallet", ticker)
			return m.renderSummary()
		}

		quantityInt := int(quantity.IntPart())
		if asset.Quantity < quantityInt {
			m.err = fmt.Errorf("insufficient quantity. You have %d shares, trying to sell %d", asset.Quantity, quantityInt)
			return m.renderSummary()
		}
	}
...
func (m *transactModel) saveTransaction() error {
	...
	transaction.Hash = parser.CalculateHash(&transaction)

	// Check for duplicate
	if _, exists := m.wallet.TransactionsByHash[transaction.Hash]; exists {
		return fmt.Errorf("duplicate transaction detected")
	}

	// Add to wallet
	m.wallet.Transactions = append(m.wallet.Transactions, transaction)
	m.wallet.TransactionsByHash[transaction.Hash] = transaction
	...
	if !exists {
		...
		m.wallet.Assets[ticker] = asset
	}

	asset.Negotiations = append(asset.Negotiations, transaction)

	// Recalculate
	m.wallet.RecalculateAssets()
